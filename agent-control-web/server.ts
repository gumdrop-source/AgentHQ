...
async function writeSystemdDropIn(agentName: string): Promise<void> {
    ...
    const lines: string[] = [
        ...
        "[Service]",
        // Per-agent telegram bot token (always — every agent uses Telegram for I/O)
        `LoadCredentialEncrypted=telegram_bot_token:/etc/agents/credentials/${agentName}_telegram_bot_token.cred`,
    ];
