...
// Refresh the systemd drop-in for an agent based on the integrations
// they've been granted any tool from. The drop-in's LoadCredentialEncrypted=
// lines are what get the credentials into $CREDENTIALS_DIRECTORY at service
// start, where the MCP server reads them.
//
// Without this, an MCP server tries to read /etc/agents/credentials/*.cred
// directly, hits the 0700-root permission, and crashes with PermissionError.
async function writeSystemdDropIn(agentName: string): Promise<void> {
    ...
    const lines: string[] = [
        ...
        "[Service]",
        // Per-agent telegram bot token (always — every agent uses Telegram).
        // systemd-creds embeds the credential name at encrypt time and refuses
        // to load if the runtime name differs from the embedded one. The cred
        // was set as <agent>_telegram_bot_token via agenthq-cred, so the
        // runtime name must match.
    ...
    for (const id of grantedServers) {
        ...
        for (const cred of (manifest.credentials ?? [])) {
            ...
            lines.push(`LoadCredentialEncrypted=${key}:/etc/agents/credentials/${key}.cred`);
        }
    }

    const dropinDir = `/etc/systemd/system/agent@${agentName}.service.d`;
    const fs = await import("node:fs");
    fs.mkdirSync(dropinDir, { recursive: true });
    fs.writeFileSync(`${dropinDir}/credentials.conf`, lines.join("\n") + "\n");
